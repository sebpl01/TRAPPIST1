def simulationSetUp(N_pl, eParameter, iParameter, diskWidth, time, timesteps):
    import rebound
    import numpy as np
    ...
    sim.add(m=.0898)
    sim.add(m=1.374 * mEarth, r=1.116 * rEarth, a=0.01154, e=0.00305473, omega=2.35156489, f=1.62005,
            inc=0.004747296)  # Trappist1b
    sim.add(m=1.308 * mEarth, r=1.097 * rEarth, a=0.01580, e=0.00055009, omega=0.01817982, f=4.448437,
            inc=0.003874631)  # Trappist1c
    sim.add(m=0.388 * mEarth, r=.788 * rEarth, a=0.02227, e=0.00563298, omega=2.64777152, f=5.293947,
            inc=0.001815142)  # Trappist1d
    sim.add(m=0.692 * mEarth, r=.920 * rEarth, a=0.02925, e=0.00632463, omega=0.81670784, f=2.183237,
            inc=0.003612832)  # Trappist1e
    sim.add(m=1.039 * mEarth, r=1.045 * rEarth, a=0.03849, e=0.00841547, omega=0.06063985, f=5.09579,
            inc=0.004537856)  # Trappist1f
    sim.add(m=1.321 * mEarth, r=1.129 * rEarth, a=0.04683, e=0.00400979, omega=0.32490512, f=4.12414,
            inc=0.004502949)  # Trappist1g
    sim.add(m=0.326 * mEarth, r=0.755 * rEarth, a=0.06189, e=0.00365005, omega=0.0054794, f=4.7124,
            inc=0.003403392)  # Trappist1h
    ...
    while sim.N < (N_pl + sim.N_active):
        a = rand_powerlaw(0, aTrappistH + hillRadiusH, aTrappistH + hillRadiusH + diskWidth)
        e = rand_rayleigh(eParameter)  # Original value 0.01
        inc = rand_rayleigh(iParameter)  # Original value 0.005
        f = rand_uniform(-np.pi, np.pi)
        p = rebound.Particle(simulation=sim, primary=sim.particles[0], m=m_pl, r=r_pl, a=a, e=e, inc=inc, Omega=0,
                             omega=0, f=f)
    ...
    times = np.linspace(0., time * 2 * np.pi, timesteps)  # Create timesteps. Ex; (0., 10000*2*np.pi, 4)

    # Create new folder
    outputFolder = "/Users/sebastianperezlopez/Desktop/Lissauer Codes/{},{},{},{},{},{}".format(N_pl, eParameter,
                                                                                                iParameter, diskWidth,
                                                                                                time, timesteps)
    ...
    for i, t in enumerate(times):
        ...
        plt.xlim(0, .1 + diskWidth)
        plt.ylim(0, .2)
        plt.title("System at t=%6.1f: " % ((sim.t) / 2 / np.pi))
        plt.xlabel("Semimajor axis (AU)")
        plt.ylabel("Eccentricity")

        plt.savefig(outputFolder + '/%s.jpg' % (sim.t), bbox_inches='tight', dpi=150)

    # Plot final system, isolated to compare with start of system

    plt.figure(figsize=(15, 10))
    plt.scatter(x=semimajorAxis, y=eccentricity, s=mass, c='black')
    plt.xlim(0, .5)
    plt.ylim(0, .2)
    plt.title("Final System (Isolated) at t=%6.1f: " % ((sim.t) / 2 / np.pi))
    plt.xlabel("Semimajor axis (AU)")
    plt.ylabel("Eccentricity")
    plt.savefig(outputFolder + '/FINAL.jpg', bbox_inches='tight', dpi=150)

    # Output data into a text file for further analysis

    f = open(outputFolder + "/outputData.txt", "a")
    i = -1
    for timestep in xy:
        i += 1
        j = -1
...
simulationSetUp(500, .00, .005, .25, 10000, 4)
simulationSetUp(500, .00, .005, .25, 10000, 4)
simulationSetUp(500, .00, .005, .25, 10000, 4)
simulationSetUp(500, .00, .005, .25, 10000, 4)
simulationSetUp(500, .00, .005, .25, 10000, 4)